...
(* CHANGE: Add a case for Shift expressions *)
	  ...
	  case eval_prog (exp,env) of
	  	NoPoints => NoPoints
		| Point (deltaX, deltaY) => Point (x + deltaX, y + deltaY)
		| Line (m, b) => Line (m, b + deltaY - m * deltaX)
		| VerticalLine(x) =>  VerticalLine(x+ deltaX)
		| LineSegment (x2, y2, x1, y1) => LineSegment (x1 + deltaX + , y1 + deltaY, x2 + deltaX, y2 + deltaY)


(* CHANGE: Add function preprocess_prog of type geom_exp -> geom_exp *)

(*To simplify the interpreter, we first preprocess expressions.*)
